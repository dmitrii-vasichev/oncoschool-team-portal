...
class AIService:
    ...
    async def parse_meeting_outcomes(
        ...
        """Parse full meeting transcript into an editable outcome draft."""
        provider = await self._get_current_provider(session, feature_key="meetings_summary")
        team_json = json.dumps(
            [
                {"name": m.full_name, "variants": m.name_variants or []}
        ...
        chunks = self._split_meeting_text(
            prepared_text, model_hint=getattr(provider, "model", None)
        )
        ...
        for idx, chunk in enumerate(chunks, start=1):
            ...
            try:
                ...
                parsed_chunks.append(ParsedMeeting(**normalized))
            except (json.JSONDecodeError, ValueError) as e:
                logger.error(
                    "Failed to parse AI outcomes chunk %s/%s as ParsedMeeting: %s\nResponse: %s",
                    idx,
                    len(chunks),
                    e,
                    response,
